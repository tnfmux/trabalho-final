...
- **Bullets:** tabela M1–M6 com AIC, BIC, RMSE, MAE; melhor modelo = M6 (SARIMAX +
  câmbio + Selic + IGP-M + Focus); RMSE = 0,1248, MAE = 0,0989.
- **Visual:** `output/figures/forecast_ipca.png` + `output/tables/comparacao_ipca.csv`.
- **Fala:** "Para o cheio, o melhor desempenho fora da amostra foi do modelo M6,
  o que sugere que as regressoras ajudaram — em especial a expectativa Focus, que
  reduziu o RMSE em mais da metade ante o SARIMA puro."

### Slide 10 — Resultados para o núcleo
- **Bullets:** tabela M1–M6; melhor modelo = M5 (SARIMAX + câmbio + Selic + IGP-M);
  RMSE = 0,1253, MAE = 0,0997.
- **Visual:** `output/figures/forecast_nucleo.png` + `output/tables/comparacao_nucleo.csv`.
- **Fala:** "No núcleo, o melhor modelo foi o M5. Como esperado, o erro de previsão
  tende a ser menor que o do cheio, e o ganho das regressoras é mais modesto, porque
  o núcleo é mais persistente e quase autorregressivo."
